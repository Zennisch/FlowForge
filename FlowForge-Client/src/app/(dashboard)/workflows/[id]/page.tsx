			...
			<section className="rounded-2xl border border-(--color-border) bg-white p-6">
				...
				{workflowQuery.data ? (
					<div className="mt-6 space-y-4">
						<div className="rounded-xl border border-(--color-border) bg-blue-50/40 p-4">
							...
							<div className="mt-3 grid gap-3 lg:grid-cols-2">
								<label className="block">
									<span className="text-xs font-medium text-(--color-text-secondary)">Payload (JSON object)</span>
									<textarea
								...
								<label className="block">
									<span className="text-xs font-medium text-(--color-text-secondary)">Idempotency key</span>
									<input
										type="text"
							...
							<div className="mt-3">
								<button
									...
									disabled={triggerWorkflowMutation.isPending || workflowQuery.isPending || workflowQuery.isError}
						...
						{triggerFormError ? (
							<p className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{triggerFormError}</p>
						) : null}

						{triggerMessage ? (
							<p className="rounded-lg bg-blue-50 px-3 py-2 text-sm text-(--color-text-secondary)">{triggerMessage}</p>
						) : null}
						...
						<WorkflowForm
							...
							submitError={updateWorkflowMutation.isError ? updateWorkflowMutation.error.message : undefined}

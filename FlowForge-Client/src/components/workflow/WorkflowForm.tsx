...
function getStringFromConfig(config: Record<string, unknown>, keys: string[]): string | undefined {
...
function omitKeys(
  ...
  return Object.fromEntries(Object.entries(config).filter(([key]) => !reservedKeys.has(key)));
}

function hasReservedKey(config: Record<string, unknown>, reservedKeys: Set<string>): boolean {
  ...
  .superRefine((values, ctx) => {
    ...
    if (values.triggerType === 'webhook' && values.webhookPath?.trim()) {
      ...
      if (!/^[A-Za-z0-9_-]+$/.test(normalizedWebhookPath)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'Webhook path may only contain letters, numbers, underscore, and hyphen',
          path: ['webhookPath'],
        });
      }
    }

    if (values.triggerType === 'webhook' && hasReservedKey(additionalConfig, webhookReservedKeys)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
...
export function WorkflowForm({
      ...
      values.triggerType === 'manual'
        ...
        : values.triggerType === 'webhook'
          ? {
              ...
              ...(values.webhookSecret?.trim() ? { secret: values.webhookSecret.trim() } : {}),
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit(internalSubmit)}>
        ...
        <div className="mt-3 grid gap-4 md:grid-cols-2">
          ...
          {triggerType === 'manual' ? (
            <div className="rounded-lg border border-dashed border-(--color-border) bg-white/70 p-3 text-xs text-(--color-text-secondary) md:col-span-2">
              Manual trigger has no required config. You can still provide optional keys in the
          ...
          {triggerType === 'schedule' ? (
            <>
              <label className="block">
                <span className="mb-1 block text-sm text-(--color-text-secondary)">
                  Cron expression (trigger.config.cron)
                </span>
                <input
              ...
              <label className="block">
                <span className="mb-1 block text-sm text-(--color-text-secondary)">
                  Timezone (optional, trigger.config.timezone)
                </span>
                <input
                ...
                <p className="mt-1 text-xs text-(--color-error)">
                  {errors.scheduleTimezone?.message}
                </p>
              </label>
            </>
          ) : null}
          ...
          <label className="block md:col-span-2">
            <span className="mb-1 block text-sm text-(--color-text-secondary)">
              Additional trigger config (JSON object)
            </span>
            <textarea
            ...
            />
            <p className="mt-1 text-xs text-(--color-error)">
              {errors.additionalTriggerConfigJson?.message}
            </p>
          </label>
        </div>
      </section>
      ...
      {submitError ? (
        <p className="rounded-lg bg-(--color-error-light) px-3 py-2 text-sm text-(--color-error)">
          {submitError}
        </p>
      ) : null}
      ...
      <div className="flex flex-wrap items-center justify-end gap-2">
        ...
        >
          {isPending
            ? 'Saving...'
            : (submitLabel ?? (mode === 'create' ? 'Create workflow' : 'Save changes'))}
        </button>
      </div>
    </form>
  );

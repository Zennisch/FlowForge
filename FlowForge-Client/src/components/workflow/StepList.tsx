	...
	return (
		<div className="space-y-6">
			<section className="space-y-3 rounded-xl border border-(--color-border) bg-blue-50/35 p-4">
				...
				{typeof errors.steps?.message === 'string' ? <ErrorText message={errors.steps.message} /> : null}
				...
				<div className="space-y-3">
					{stepFields.map((field, index) => (
						<div key={field.formId} className="rounded-lg border border-(--color-border) bg-white p-3">
							<div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
								...
								<label className="block text-sm">
									<span className="mb-1 block text-xs text-(--color-text-secondary)">Max attempts</span>
									<input
										type="number"
										min={1}
										max={10}
			...
			<section className="space-y-3 rounded-xl border border-(--color-border) bg-blue-50/35 p-4">
				<div className="flex items-center justify-between gap-3">
					...
					<button
						type="button"
						onClick={() => appendEdge({ from: stepIdOptions[0] ?? '', to: stepIdOptions[0] ?? '', condition: '' })}
				...
				{typeof errors.edges?.message === 'string' ? <ErrorText message={errors.edges.message} /> : null}
					...
					{edgeFields.map((field, index) => (
						<div key={field.formId} className="rounded-lg border border-(--color-border) bg-white p-3">
							<div className="grid gap-3 md:grid-cols-3">
								...
								<label className="block text-sm">
									<span className="mb-1 block text-xs text-(--color-text-secondary)">Condition</span>
									<input

				...
				<tbody className="divide-y divide-(--color-border) bg-white">
					{steps.map((step) => (
						<tr key={step.id || step.stepId}>
							<td className="px-4 py-3 text-sm font-medium text-(--color-text-primary)">{step.stepId}</td>
							...
							<td className="max-w-[16rem] truncate px-4 py-3 text-sm text-(--color-text-secondary)">{formatValue(step.input)}</td>
							<td className="max-w-[16rem] truncate px-4 py-3 text-sm text-(--color-text-secondary)">{formatValue(step.output)}</td>
							<td className="px-4 py-3 text-sm text-red-700">{step.error ?? '-'}</td>
							<td className="px-4 py-3 text-sm text-(--color-text-secondary)">{formatDateTime(step.startedAt)}</td>
							<td className="px-4 py-3 text-sm text-(--color-text-secondary)">{formatDateTime(step.completedAt)}</td>
						</tr>
					))}
				</tbody>
			</table>

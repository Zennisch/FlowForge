  ...
  return createPortal(
    <AnimatePresence>
      {!disabled && isOpen && (
        ...
        >
          {searchable && (
            <li className={cn("sticky top-0 z-10", theme.selectBg, theme.selectSearchBorder, LAYOUT.search.container)}>
              <div className="relative">
                <span
                  className={cn("absolute inset-y-0 left-0 flex items-center", theme.selectIconColor, LAYOUT.search.iconLeft)}
                >
                ...
                <input
                  ref={searchInputRef}
                  type="text"
                  className={cn("w-full rounded border text-sm", theme.selectSearchInput, LAYOUT.search.input)}
                  placeholder="Search..."
                  value={searchQuery}
                  onKeyDown={onKeyDown}
          ...
          {isLoading ? (
            ...
            <div className={cn(theme.selectEmptyText, "text-sm text-center", LAYOUT.emptyState.padding)}>No options found</div>
          ) : (

...
function buildExecutionListQueryParams(query: ExecutionListQuery): Record<string, string | number | boolean> {
...
function buildExecutionEventsQueryParams(query: ExecutionEventsQuery): Record<string, string | number> {
...
export const executionsApi = {
	...
	async releaseLegalHold(id: string): Promise<ReleaseExecutionLegalHoldResponse> {
		const { data } = await apiClient.delete<RawLegalHoldResponse>(
			`/executions/${id}/legal-hold`,
		);
		return normalizeLegalHoldResponse(data);
	},

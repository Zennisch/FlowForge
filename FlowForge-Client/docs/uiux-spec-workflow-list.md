...
*   **Logic (dựa trên contract):** 
    *   Nếu `status === 'active'`: Nút có màu xanh dương (Brand Blue) nhạt, hover lên đậm hơn. Tooltip: *"Trigger Workflow"*.
    *   Nếu `status === 'inactive'`: Nút bị `disabled`, màu xám mờ. Tooltip: *"Workflow is inactive"*.
...
*   *Hiển thị icon kèm text nhỏ để user biết workflow này được chạy theo cách nào.*
...
*   **Hành động Trigger (Khi bấm nút Play ở Cột 1):**
    ...
    *   Nếu thành công: Hiển thị Toast Notification (Góc phải dưới): *"Execution triggered successfully. [View Run ->]"*
*   **Empty State (Trạng thái rỗng):** Nếu danh sách không có dữ liệu (0 workflows). Hiển thị một khung đứt nét (Dashed border) ở giữa màn hình. Bên trong là Icon Diagram mờ, câu text: *"No workflows found. Build your first asynchronous process."* và một nút "Create Workflow".
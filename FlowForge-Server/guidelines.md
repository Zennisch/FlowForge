...
- Optional webhook secret: set `trigger.config.secret`; incoming requests must provide secret via `x-webhook-secret` header (preferred) or JSON body `secret`, otherwise API returns `401 Unauthorized`
...
*Last updated: 2026-03-21 — Added execution timeout policies + watchdog safeguards for stuck workflows. `ExecutionService` now persists execution timeout policy/deadline, `StepStateService` assigns per-step runtime deadlines, and new `ExecutionWatchdogService` periodically fails timed-out running steps/executions then triggers compensation. Added regression tests in execution-watchdog.service.spec.ts and updated execution/step-state compensation specs for timeout behavior.*

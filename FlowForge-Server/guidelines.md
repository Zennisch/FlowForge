...
*Last updated: 2026-03-19 — Webhook secret verification implemented. Added support for `trigger.config.secret` on webhook workflows with secure secret comparison in ExecutionService. WebhookController extracts provided secret from `x-webhook-secret` header (preferred) or JSON body `secret` (fallback), strips body secret from persisted payload, and returns `401 Unauthorized` for missing/invalid secret when configured. Unit tests updated in workflow.service.spec.ts and execution.service.spec.ts. Targeted test suites pass and build is successful.*

...
*Last updated: 2026-03-21 — Scheduled trigger deduplication across multi-instance deployments implemented. Added MongoDB-backed scheduler leadership lease (`scheduler_locks`) in WorkflowSchedulerService; only the leader instance registers and runs cron jobs, while followers clear local jobs and skip triggering. Added regression tests in workflow-scheduler.service.spec.ts for leader/follower lock behavior.*
